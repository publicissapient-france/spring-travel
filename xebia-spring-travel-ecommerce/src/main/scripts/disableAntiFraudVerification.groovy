...
def cli = new CliBuilder( usage: 'groovy disableAntiFraudVerification [-h] -s servername -p port')
...
println("Disable Anti Fraud Verification on $jmxClient")
...
server.queryNames(new ObjectName('travel-ecommerce:name=AntiFraudService,type=AntiFraudService,host=localhost,path=/xebia-spring-travel-ecommerce'), null).each { name ->
    ...
    bookingAction.EnableAntiFraudService = false

}
jmxClient.close()
println "Anti Fraud Verification successfully disabled. Bye"
