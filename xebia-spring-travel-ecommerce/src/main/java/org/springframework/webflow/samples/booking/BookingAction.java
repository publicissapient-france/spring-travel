	...
	public Event submitPayment(RequestContext context, Booking booking)
		...
		try {
			try {
				...
				auditLogger.info("AntiFraud granted booking "
						+ toXmlString(booking));

			} catch (SuspiciousBookingException e) {
				auditLogger.error("AntiFraud rejected booking "
						+ toXmlString(booking));

				return processException(context, e, "Suspicious order.");

			} catch (RuntimeException e) {
				auditLogger.error("Exception invoking AntiFraud on booking "
						+ toXmlString(booking));
				throw e;
			}

			creditCardService.purchase(new MonetaryAmount(booking.getTotal(),
					Currency.getInstance("USD")), booking.createOrder(),
			...
			return processException(context, e,
					"You payment was not processed. Please contact customer service.");

		} catch (GatewayClientException e) {
			return processException(context, e,
					"Transaction failed at this time due to a payment gateway error.");

		} catch (Throwable t) {
			return processException(context, t,
					"Unexpected error. Please contact customer service.");
		} finally {
			this.totalDurationInNanos
